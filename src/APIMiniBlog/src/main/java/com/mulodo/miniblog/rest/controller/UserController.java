...
public class UserController {
	 ...
	 *	
	 *	@param	us : UserService from datasource
	 *
	 *	@return void
	 */
	@Autowired
    ...
    public Response addUser( @FormParam("username") String username,
		...
		try {
			...
			if(check == true){
				...
				jsonObject = BuildJSON.buildReturn(new Meta(Constraints.CODE_2000, Constraints.CODE_2010), null);
				return Response.status(200).entity(jsonObject.toString()).build();
			}
			//check exists email in database
			check = this.userService.isEmailExits(email.replaceAll(Constraints.REGEX_END_WHITESPACE, ""), null);
			if(check == true){
	...
	/**
	 *  update_user is api that user use for update new information user in miniblog
	 *	
	 *  @param  firstname : firstname of user
	 *  @param  lastname : lastname of user
	 *  @param  email : email of user
	 ...
	 *	
	 *  @param  name : name is username, firstname or lastname of user.
	 *     
	 *	@return Response
	 *	
	 */
    ...
    public Response  findByName(@HeaderParam(Constraints.ACCESS_KEY) String access_key, 
		...
		try {
			// get list user from userservice match with name (firstname, lastname, firstname)
	 ...
	 *	
	 *  @param  access_key : access key in token table
	 *    
	 *	@return Response
	 *	
	 */
    ...
    public Response  login(@FormParam("username") String username, 
		...
		try {
			...
			if(user != null){
				//if valid login, create new token and set new data to this token
